...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		List<UserFormBean> userFormMaps = userMap.mapper().findUserByAccName(userMap);
		if (userFormMaps.size() != 0) {
			...
			session.setAttribute(SysConsts.ORG_CODE, userFormMaps.get(0).get(SysConsts.ORG_CODE));
			session.setAttribute(SysConsts.ORG_TYPE, userFormMaps.get(0).get(SysConsts.ORG_TYPE));
			session.setAttribute(SysConsts.ROLE_NAME, userFormMaps.get(0).get(SysConsts.ROLE_NAME));
			session.setAttribute(SysConsts.ROLE_KEY, userFormMaps.get(0).get(SysConsts.ROLE_KEY));
			session.setAttribute(SysConsts.STATION_NAME, userFormMaps.get(0).get(SysConsts.STATION_NAME));
